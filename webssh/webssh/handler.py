...
from webssh.utils import (UnicodeType, is_ip_hostname, is_same_primary_domain,
                          is_valid_encoding, is_valid_hostname,
                          is_valid_ip_address, is_valid_port, to_bytes, to_int,
                          to_ip_address, to_str)
...
# accept environment variable values for login parameters
ENV_SSH_HOST = os.environ.get("SSH_HOST", "localhost")
...
ENV_SSH_CONTENT_FLAVOR = os.environ.get("SSH_CONTENT_FLAVOR", "undefined")
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def get(self):
        self.render(
            ...
            debug=self.debug,
            font=self.font,
            ssh_host="2222" if ENV_SSH_HOST is None else ENV_SSH_HOST,
            ...
            content_flavor="undefined" if ENV_SSH_CONTENT_FLAVOR is None else ENV_SSH_CONTENT_FLAVOR,
        )
    ...
    def post(self):
        ...
        if workers and len(workers) >= options.maxconn:
            raise tornado.web.HTTPError(429, "Too many live connections.")
